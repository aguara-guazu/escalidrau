...
While the app is open it exposes an MCP server at `http://localhost:3580/mcp` with these tools: `get_scene`, `get_layout`, `get_library`, `view_library`, `add_library_item`, `add_elements`, `connect_elements`, `get_canvas_style`, `set_canvas_style`, `update_elements`, `move_elements`, `delete_elements`, `import_mermaid`, `export_mermaid`, `view_canvas`, `export_image`, `wait_for_user_changes`. The **MCP** menu also installs the `escalidrau` skill (`skills/escalidrau/SKILL.md`) for Claude Code and Codex: the layout method the agent follows to keep diagrams tidy — grid spacing, label clearance, anchored connectors.
